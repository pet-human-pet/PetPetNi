...
// 使用密碼驗證 composable
const { rules, score, isValid, strengthLevel, getProgressColor } = usePasswordValidator(password)
...
const handleRegister = async () => {
  ...
  // 驗證密碼是否為 ASCII 字符（防止重音符號）
  if (!isASCIIPassword(password.value)) {
  ...
  // 驗證密碼強度
  if (!isValid.value) {
    passwordError.value = '密碼強度不足，請滿足至少 4 項安全要求。'
    return
  }
  ...
  <div class="w-full max-w-md rounded-3xl border-none bg-white p-8 shadow-xl md:p-12">
    ...
    <form class="space-y-6" novalidate @submit.prevent="handleRegister">
      ...
      <div class="space-y-2">
        ...
        <!-- 密碼強度視覺化 -->
        <div v-if="password" class="space-y-3 pt-2">
          <!-- 強度進度條 -->
          <div class="space-y-1">
            ...
            <div class="flex h-1.5 w-full gap-1">
              <div
                ...
                :class="index <= score ? getProgressColor : 'bg-gray-200'"
              ></div>
            </div>
          </div>

          <!-- 僅顯示未通過的規則（紅色警告） -->
          <ul v-if="rules.filter((r) => !r.met).length > 0" class="space-y-1.5 text-xs">
            <li
              v-for="rule in rules.filter((r) => !r.met)"
              :key="rule.key"
              class="flex items-center gap-2 text-red-500 transition-all duration-200"
            >
              <!-- 警告圖示 -->
        ...
        <BaseInput
          ...
          @blur="
            confirmPasswordError =
              confirmPassword && password !== confirmPassword ? '兩次密碼輸入不一致' : ''
          "
        />
      </div>
