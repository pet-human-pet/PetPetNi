  ...
  <section class="mt-4 hidden w-full rounded-2xl border bg-white p-4 md:block">
    <div class="flex items-start gap-3">
      ...
      <div class="min-w-0 flex-1">
        <textarea v-model="content" class="min-h-16 w-full resize-none bg-transparent text-sm outline-none"
          placeholder="輸入文字" :maxlength="maxLength" />

        <!-- 圖片預覽區 -->
        <div v-if="images.length > 0" class="mt-3 flex flex-wrap gap-2">
          <div v-for="(img, index) in images" :key="img.id"
            class="relative group h-24 w-24 overflow-hidden rounded-xl border border-zinc-200">
            <img :src="img.url" class="h-full w-full object-cover" />
            <button type="button"
              class="absolute right-1 top-1 grid h-6 w-6 place-items-center rounded-full bg-black/50 text-white hover:bg-black/70"
        ...
        <div class="mt-3 flex items-center justify-between">
          <div class="flex items-center gap-5 text-zinc-700">
            ...
            <button type="button" class="grid h-9 w-9 place-items-center rounded-full hover:bg-zinc-100"
              aria-label="Image" @click="triggerImageUpload">
              🖼️
            </button>
            <button type="button" class="grid h-9 w-9 place-items-center rounded-full hover:bg-zinc-100"
              aria-label="Hashtag">
          ...
          <div class="relative flex items-center gap-3">
            ...
            <!-- 下拉選單 -->
            <div v-if="audienceOpen" class="absolute right-0 top-11 z-10 w-40 rounded-xl border bg-white p-1 shadow">
              <button type="button" class="w-full rounded-lg px-3 py-2 text-left text-sm hover:bg-zinc-100"
      ...
      <section class="w-full rounded-2xl bg-white p-4 shadow-lg">
        ...
        <div class="mt-3">
          <textarea ref="textareaRef" v-model="content"
            class="w-full resize-none bg-transparent text-base leading-7 outline-none" placeholder="輸入文字"
        ...
        <div v-if="images.length > 0" class="mt-3 flex gap-2 overflow-x-auto pb-2">
          <div v-for="(img, index) in images" :key="img.id"
            class="relative shrink-0 h-24 w-24 overflow-hidden rounded-xl border border-zinc-200">
            <img :src="img.url" class="h-full w-full object-cover" />
            <button type="button"
              class="absolute right-1 top-1 grid h-6 w-6 place-items-center rounded-full bg-black/50 text-white hover:bg-black/70"
              @click="removeImage(index)">
        ...
        <div class="mt-4 flex items-center justify-between gap-3">
          ...
          <div class="flex items-center gap-3 text-zinc-700">
            <button type="button" class="grid h-10 w-10 place-items-center rounded-full bg-zinc-100" aria-label="Upload"
              @click="triggerImageUpload">
              🖼️
            </button>
            <button type="button" class="grid h-10 w-10 place-items-center rounded-full bg-zinc-100"
              aria-label="Hashtag">
              <span class="text-lg font-semibold">#</span>
            </button>
          </div>
          ...
          <div class="flex items-center gap-3">
            ...
            <button type="button"
              ...
              :disabled="!canSubmit" @click="submitAndClose">
              {{ editing ? '更新' : '發布' }}
            </button>
          </div>
        </div>
